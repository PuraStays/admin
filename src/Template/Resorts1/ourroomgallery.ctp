...
<h1>
            ...
            <?= __('Set our room Gallery Image priority') ?>
</h1>
		...
		$t=$_POST['t1'];
		$query="INSERT INTO `our_room_gallery`(`r_id`, `image`, `r_order`) VALUES ('$id','$checkbox1','$t');";
		//print_r($query);
...
<?php
		...
		$qry1="SELECT * FROM `our_room_gallery` where r_id='$id'";
		$i=0;
		...
		{
      $qry2="UPDATE `our_room_gallery` SET `r_order`='$tsa[$i]' WHERE id='$idd[$i]';";
	 //print_r($qry2);
  ...
  <?php
      ...
      $qry3="delete FROM `our_room_gallery` WHERE id='$i_id'";
	  //print_r($query);
