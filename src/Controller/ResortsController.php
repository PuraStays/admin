...
class ResortsController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->set('resorts', $this->Resorts->find('all'));
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if(count($this->request->data['feature_id'])>0){
             ...
             if(count($this->request->data['Cafe_Gallery'])>0){
                foreach ($this->request->data['Cafe_Gallery'] as $gallery)
                    $resort->Cafe_Gallery = $resort->Cafe_Gallery. $gallery.', ' ;
            }
        ...
        $About_Image = $this->Resorts->Images->find('list', 
            ['keyField' => 'Id',
            'valueField' => 'Title']);
        ...
        $this->set(compact('resort', 'features', 'testimonials', 'galleries', 'things_to_do_id', 'nearbyplaces_id', 'About_Image','Banner_Image', 'Banner_Video', 'Tags', 'Our_Room_Gallery', 'programs_id', 'Cafe_Gallery'));
        $this->set('_serialize', ['resort']);
    }
    ...
    public function edit($id = null)
    {
        ...
        $resort->room_id = explode(", ", $resort->room_id);
        ...
        $resort->Cafe_Gallery = explode(", ", $resort->Cafe_Gallery);
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if(count($this->request->data['room_id'])>0){
                foreach ($this->request->data['room_id'] as $room)
                    $resort->room_id = $resort->room_id. $room.', ' ;
            }
        ...
        $rooms = $this->Resorts->resorts_rooms->find('list', 
            ['keyField' => 'Id',
            'valueField' => 'RoomType',
            'conditions' => ['resorts_rooms.resort_id =' => $id, 'resorts_rooms.Status =' => 1 ],
            ]);
       ...
       $Our_Room_Gallery = $this->Resorts->Images->find('list', 
        ...
        $this->set(compact('resort', 'features', 'testimonials', 'galleries', 'things_to_do_id', 'nearbyplaces_id', 'About_Image', 'Banner_Image', 'Banner_Video', 'Tags', 'Our_Room_Gallery', 'programs_id', 'Cafe_Gallery', 'rooms'));
